...
export class DataService {
  ...
  public getGroupedPandemicData(groupBy: string = "year", country: string = '', region: string = ''): Observable<Array<Metrics>> {
    let params = '';    
    if (country) {
      params += `&country=${country}`;
    }
    ...
    let columns = "year,score,gdp,social_support,hle,freedom,generosity,corruption,positive_affect,negative_affect"
    return this.http.get<Array<Metrics>>(`data-ingestion/ingested/processed-data/pandemic/grouped?group_by=${groupBy}&columns=${columns}${params}`);
  }
